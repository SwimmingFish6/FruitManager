...
import cn.edu.zju.bean.FruitReserveStoreBean;
...
public class SalerStoreController {
    ...
    @RequestMapping("/reserves/{typeId}/list")
    public ResponseEntity<List<FruitReserveStoreBean>> getReversesDetailByFruitType(@PathVariable  String typeId){
        List<StoreEntity> stores = getStoreEntity(Long.valueOf(typeId));
        List<FruitReserveStoreBean> data = new ArrayList<>();
        for(StoreEntity store: stores){
            CurrentStorageEntity storage = storeService.findStorageByStoreIdAndTypeId(store.getStoreId(),Long.valueOf(typeId));
            FruitReserveStoreBean bean = new FruitReserveStoreBean();
            bean.setStoreId(store.getStoreId());
            bean.setStoreName(store.getStorename());
            bean.setAddress(store.getAddress());
            bean.setRank(storage.getRank());
            bean.setVolume(storage.getVolume());
            data.add(bean);
        }
        return new ResponseEntity<List<FruitReserveStoreBean>>(data,HttpStatus.OK);
    }
